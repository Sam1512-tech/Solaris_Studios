export const studio = {
  name: "Solaris Studios",
  tagline: "Creative Branding & Digital Agency",
  email: "hello@solarisstudios.co.in",
  instagram: "@solaris_studios_",
  instagramUrl: "https://instagram.com/solaris_studios_",
  phone: "+91 9187416656",
  phoneHref: "tel:+919187416656",
  whatsappNumber: "919187416656",
  whatsappHref: "https://wa.me/919187416656",
  location: "Bangalore, India",
  yearFounded: 2026,
};

export const team = [
  {
    name: "Samarth Vaibhav",
    role: "Web Development Architect",
    bio: "Leads the studio's technical practice — architecture, performance, and the engineering behind every brand we ship.",
    phone: "+91 6366 26 2628",
    phoneHref: "tel:+916366262628",
    initial: "SV",
  },
  {
    name: "Vismaya",
    role: "Frontend Experience Designer",
    bio: "Shapes interaction, motion, and the small details that decide how a product feels in someone's hands.",
    phone: "+91 88617 27060",
    phoneHref: "tel:+918861727060",
    initial: "V",
  },
  {
    name: "Dhanush",
    role: "Creative Systems Developer",
    bio: "Builds the systems that hold a brand together — components, patterns, and the connective tissue between brand and product.",
    phone: "+91 63804 45641",
    phoneHref: "tel:+916380445641",
    initial: "D",
  },
  {
    name: "Likith",
    role: "Visual Communications Designer",
    bio: "Carries the visual language across brand, web, and content — from identity systems to launch-ready assets.",
    phone: "+91 81057 73035",
    phoneHref: "tel:+918105773035",
    initial: "L",
  },
];

export const services = [
  {
    slug: "brand-identity",
    title: "Brand Identity",
    body: "A visual direction that holds up across a launch, a press hit, and a quiet Tuesday morning.",
    detail: "Logo systems, art direction, guidelines, launch assets.",
    deliverables: ["Logo systems", "Art direction", "Guidelines", "Launch kit"],
  },
  {
    slug: "web-experience",
    title: "Web Experience",
    body: "Sharp, fast websites with clear structure and motion that earns its place.",
    detail: "Next.js builds, interaction systems, motion design, technical QA.",
    deliverables: ["Next.js builds", "Interaction systems", "Motion design", "Technical QA"],
  },
  {
    slug: "content-creation",
    title: "Content Creation",
    body: "A practical content system so the brand stays active without burning out your team.",
    detail: "Campaign concepts, social systems, copy direction, production planning.",
    deliverables: ["Campaigns", "Social systems", "Copy direction", "Production planning"],
  },
  {
    slug: "brand-strategy",
    title: "Brand Strategy",
    body: "Positioning, voice, and naming, so every decision after has a reason behind it.",
    detail: "Positioning, voice, naming, messaging.",
    deliverables: ["Positioning", "Voice", "Naming", "Messaging"],
  },
  {
    slug: "seo",
    title: "SEO & Discovery",
    body: "Technical SEO and content architecture for the kind of growth that does not depend on ads.",
    detail: "Technical SEO, content architecture, schema, search strategy.",
    deliverables: ["Technical SEO", "Content architecture", "Schema", "Search strategy"],
  },
];

export const testimonials = [
  {
    quote: "Solaris understood what we were trying to say before we did. Our launch finally felt like us.",
    author: "Founder",
    role: "Coaching program (placeholder)",
  },
  {
    quote: "Sharp work. They held the standard from the first call all the way to launch day.",
    author: "Co-founder",
    role: "D2C brand (placeholder)",
  },
  {
    quote: "It is rare to find one studio that can think across brand, web, and content. Solaris does.",
    author: "Marketing lead",
    role: "Services brand (placeholder)",
  },
];

export const marqueeWords = [
  "Brand Identity",
  "Web Experience",
  "Content Systems",
  "Brand Strategy",
  "SEO & Discovery",
];

export const processStages = [
  {
    title: "Discover",
    body: "We get close to the business, the audience, the offer, and what has been difficult to explain so far.",
    deliverables: ["Stakeholder interviews", "Audience review", "Competitive scan", "Brief alignment"],
    duration: "Week 1",
  },
  {
    title: "Position",
    body: "We turn what we learn into a clear direction: who you are speaking to, what you stand for, and what needs to be said first.",
    deliverables: ["Positioning statement", "Messaging hierarchy", "Voice notes", "Naming (if needed)"],
    duration: "Week 1–2",
  },
  {
    title: "Design",
    body: "We build the visual and verbal system: identity, layouts, language, content direction, and the details that make it feel like you.",
    deliverables: ["Identity system", "Page designs", "Copy direction", "Component library"],
    duration: "Week 2–4",
  },
  {
    title: "Build",
    body: "We bring the website or campaign system to life with clean implementation, useful motion, and careful testing.",
    deliverables: ["Next.js build", "Motion + interaction", "Content load", "Cross-device QA"],
    duration: "Week 4–6",
  },
  {
    title: "Launch",
    body: "We help prepare the pieces your team needs to go live with confidence, from assets to messaging to final QA.",
    deliverables: ["Launch checklist", "Analytics + SEO", "Launch comms", "Handoff docs"],
    duration: "Week 6",
  },
  {
    title: "Improve",
    body: "After launch, we look at what is working, what people respond to, and what should be sharpened next.",
    deliverables: ["Performance review", "Iteration sprint", "Content updates", "Roadmap"],
    duration: "Ongoing",
  },
];

export const projects = [
  {
    title: "Arpitha Abhishek",
    category: "Website",
    status: "Live website",
    summary: "A focused manifestation-program landing page with registration and Razorpay payment flow.",
    description: "A website for a creator and coach, structured around Arpitha's personal transformation story, program details, student testimonials, FAQs, and a clear registration-to-payment path through Razorpay.",
    url: "https://arpithaabhishek.com",
    domain: "arpithaabhishek.com",
    visual: "website",
    tags: ["Coach website", "Program landing page", "Razorpay integration"],
  },
  {
    title: "Earthiva",
    category: "Website",
    status: "Live website",
    summary: "A sustainability-led website for eco-friendly bricks and responsible construction.",
    description: "A website positioning Earthiva around eco-friendly brick manufacturing, greener materials, high-performance products, and a more responsible way to build.",
    url: "https://earthiva.in",
    domain: "earthiva.in",
    visual: "website",
    tags: ["Sustainable brand", "Product website", "Eco-friendly bricks"],
  },
  {
    title: "S S Motors",
    category: "Content Strategy / Social Media",
    status: "Content system",
    summary: "A practical social content system for keeping the brand active, consistent, and easier to follow.",
    description: "Content strategy and social media handling shaped around regular communication, cleaner planning, and a more dependable brand presence.",
    visual: "social",
    tags: ["Content strategy", "Social media handling", "Planning system"],
  },
  {
    title: "Kishan Plastic Industries",
    category: "Product Catalog / Photography",
    status: "Print catalog",
    summary: "A full product photography shoot and printed catalog system for a plastics manufacturer.",
    description: "An end-to-end catalog programme — product photography, art direction, layout, and print-ready files — built to present the full range with clarity and consistency across sales channels.",
    visual: "catalog",
    tags: ["Product photography", "Catalog design", "Print system", "Art direction"],
  },
];
